...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins)) {
      ...
      if (money == 0) //No change given, therefore only one solution
        ...
        val (left, right) = parallel(parCountChange(money, coins.tail, threshold), parCountChange(money - coins.head, coins, threshold)) //without using the first coin + using at least one of the
        left + right
      }
    }
  }
  ...
  def moneyThreshold(startingMoney: Int): Threshold =
    (money: Int, coins: List[Int]) => money <= (2 * startingMoney) / 3

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold =
    (money: Int, coins: List[Int]) => coins.length <= (2 * totalCoins) / 3
